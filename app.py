...
import google.genai as genai  # Added: Import genai client
...
gemini_api_key = gemini_api_key_input if gemini_api_key_input else os.getenv(
    'GEMINI_API_KEY')
...
openai_api_key = openai_api_key_input if openai_api_key_input else os.getenv(
    'OPENAI_API_KEY')
...
if app_config["AI_SERVICE_TYPE"] == "gemini":
    if not gemini_api_key:
        st.error(
            "GEMINI_API_KEY not found. Please enter it in the sidebar or set it as an environment variable.")
        st.stop()
    ai_service = GeminiService(
        model_name=app_config["GEMINI_MODEL_NAME"], client=genai.Client(api_key=gemini_api_key))
    ...
    if not openai_api_key:  # Only for demonstration, in a real scenario you'd need the key for OpenAI
        st.warning(
            "OPENAI_API_KEY not found. OpenAI service is currently a placeholder.")
    # Passing key for placeholder
    ai_service = OpenAIService(
        model_name=app_config["OPENAI_MODEL_NAME"], client=openai_api_key)
else:
...
search_query = st.text_input(
    "Enter Job Search Query", "DevOps Engineer for climate impact")

# 4. File Uploader for context documents
uploaded_file = st.file_uploader("Upload Context Document (e.g., Resume, Job Description)", type=[
                                 "png", "jpg", "jpeg", "pdf", "txt", "md", "docx"])
...
if uploaded_file is not None:
    ...
    if uploaded_file.size > MAX_FILE_SIZE:
        st.error(
            f"File is too large. Max size is {MAX_FILE_SIZE / (1024 * 1024):.0f} MB.")
        ...
        if uploaded_file.type in ["image/jpeg", "image/png"] or uploaded_file.name.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            st.error(
                "Unsupported file type. Please upload PNG, JPG, JPEG, PDF, TXT, MD, or DOCX.")
            uploaded_file = None  # Invalidate file
    ...
    if context_text:
        if not context_text.strip():
            st.error(
                "Extracted text content is empty. Please upload a document with content.")
            context_text = None
        elif len(context_text.strip()) < MIN_TEXT_CHAR_COUNT:
            st.error(
                f"Extracted text content has too little content ({len(context_text.strip())} characters). Please upload a document with at least {MIN_TEXT_CHAR_COUNT} characters.")
            context_text = None
    ...
    if ai_service is None:
        ...
        with st.spinner(f"Searching for job leads using {app_config['AI_SERVICE_TYPE']}..."):
            ...
            if leads:
                ...
                st.info(
                    "No job leads were returned for your query. Try a different search term or check your API key.")
