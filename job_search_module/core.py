...
def find_job_leads(
    ...
    if context_image_filepath:
        ...
        try:
            ...
            content_parts.append(Part.from_bytes(data=file_bytes, mime_type=mime_type))
            content_parts.append(Part(text="Use the uploaded image as a resume or context document to refine and prioritize your job search results."))
    ...
    content_parts.append(Part(text=user_query))

    # 3. Define the Structured Response
    single_job_lead_schema = Schema(
        type="object",
        properties={
            "title": Schema(type="string", description="The exact job title."),
            "company": Schema(type="string", description="The name of the hiring company."),
            "description": Schema(type="string", description="A concise, 1-2 sentence summary of the primary responsibilities and how they align with the user's context/query."),
            "link": Schema(type="string", description="The direct URL to the job posting."),
        },
